...
export function activate(context: vscode.ExtensionContext) {
	...
	function getSvgOutput(easingFunctionInput: string): string {
		...
		let markup = `<svg onClick="ddd()" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 ${svgW} ${svgH}" width="${svgW}px" height="${svgH}px">
            <style type="text/css">
                svg {
                    background-color: ${bg};
                    color: ${color};
	...
	vscode.window.onDidChangeActiveTextEditor(editor => {
		console.log("from onDidChangeActiveTextEditor", editor);
	...
	vscode.workspace.onDidChangeTextDocument(event => {
		console.log("from onDidChangeTextDocument", event);
